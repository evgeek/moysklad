...
- Форматтер для хелпера `Evgeek\Moysklad\Tools\Meta` автоматом устанавливается из класса `Evgeek\Moysklad\MoySklad`, то есть нет нужды задавать его вручную. Однако, если необходимо задать другой формат для меты - `Meta::setFormat()` по-прежнему работает.
