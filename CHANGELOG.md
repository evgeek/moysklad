...
- Переотправка неудачных запросов: конструктор `Evgeek\Moysklad\Http\GuzzleSender` в качестве первого аргумента принимает количество попыток отправки.
- В качестве второго аргумента конструктор `Evgeek\Moysklad\Http\GuzzleSender` принимает максимальный размер тела ответа в исключениях от неудачных запросов.
