...
Тело запроса формируется из свойств сущности. Если в сущности не задан id, считается, что она не существует в Моём Складе, и для неё доступен только метод `create()`. Если id задан, доступны все прочие CRUD-методы, кроме `create()`. Все методы отправки запросов обновляют объект сущности и возвращают его же.
