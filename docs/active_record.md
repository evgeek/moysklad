...
Если родительская сущность является коллекцией (последний сегмент url не является guid), это можно сделать в следующих форматах:
...
Если же родительская сущность является конкретным объектом (последний сегмент url является guid), при инициализации требуется задать её id:
